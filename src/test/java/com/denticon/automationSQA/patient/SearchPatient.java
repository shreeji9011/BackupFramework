...
//	@Test(description = "Verify Search Patient functionality")
	public boolean verifySearchPatientPage() {
		try {
			...
			if (!driver.getCurrentUrl().contains("PatientView")) {
...
//	@Test(description = "Verify Patient tab functionality")
	public boolean verifyPatientMedicaidID() {
		...
		try {
			...
			flag = CommonMethods.sendText(ISearchPatient.TXT_searchText, "15");
	...
	public static boolean selectNthRowFromResults(By Table, int n) {
		...
		try {
			...
			WebElement ele = DriverManager.getDriver().findElement(By
					.xpath("//tr[@id='dgSearch_ctl00__" + (n - 1) + "']//a[@id='dgSearch_ctl00_ctl04_lnkPatientID']"));
	...
	public static boolean selectNthRowFromResponsiblePartyResults(By Table, int n) {
		...
		try {
			...
			WebElement ele = DriverManager.getDriver()
					.findElement(By.xpath("//tr[@id='dgSearch_ctl00__" + (n - 1) + "']//a"));
			wait.until(ExpectedConditions.visibilityOf(ele));
			ele.click();
			return true;
		} catch (Exception eTO) {
